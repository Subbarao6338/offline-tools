...
fun CategoryFallbackDispatcher(navController: NavHostController, tool: Tool, aiApiKey: String) {
    ...
    when (category) {
        ...
        "Media", "GIF & Animation" -> MediaDispatcher(navController, tool)
        "Health" -> HealthScreen(navController, tool.name)
        "Device", "Security", "Utilities" -> if (route == "thermal_info") SensorDataScreen(navController, tool.name) else SystemLabScreen(navController, tool.name)
